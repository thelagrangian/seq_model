...
parser.add_argument('--validseqlen', type=int, default=300,
                    help='valid sequence length (default: 300)')
...
# amendments.txt  constitute.txt  federalist.txt


file = open('./data/federalist.txt').read()
file_len = len(file)
valfile = open('./data/amendments.txt').read()
valfile_len = len(valfile)
testfile = open('./data/constitute.txt').read()
...
train_data = batchify(char_tensor(file), args.batch_size, args)
val_data = batchify(char_tensor(valfile), 1, args)
test_data = batchify(char_tensor(testfile), 1, args)
...
def evaluate(source):
    ...
    for batch, i in enumerate(range(0, source_len - 1, args.validseqlen)):
        ...
        for x in range(len(final_output)):
            dist = final_output[x].data.view(-1).div(0.8).exp()
        ...
        for x in range(len(final_target)):
            nextChar = string.printable[final_target[x]]
            aux_str2 += nextChar

        n = min(len(aux_str), len(aux_str2))
        upper = 0
        for y in range(n):
            if  aux_str[y] == aux_str2[y]:
                upper += 1
