...
def data_generator(args):
    ...
    testfile = open('./data/amendments.txt').read()
    valfile  = open('./data/constitute.txt').read()
    file_len = len(file)
...
def char_tensor(sti):
    st = sti[0:len(sti)]
    tensor = torch.zeros(len(st)).long()
    for i in range(len(st)):
        try:
            tensor[i] = string.printable.index(st[i])
        except ValueError:
            print('{} not found in the search space'.format(st[i]))
            continue
